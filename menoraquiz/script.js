...
let audioCtx = null;
let soundEnabled = localStorage.getItem("menoraquiz_sound") !== "off";
...
function showResult() {
    ...
    const highscore = Number(localStorage.getItem("menoraquiz_highscore") || 0);
    const newHighscore = score > highscore ? score : highscore;
    if (newHighscore !== highscore) {
        localStorage.setItem("menoraquiz_highscore", String(newHighscore));
    }
...
soundToggleButton.addEventListener("click", () => {
    ...
    localStorage.setItem("menoraquiz_sound", soundEnabled ? "on" : "off");
    syncSoundToggleUI();
    unlockAudio();
    playSound("tap");
});
